...
chrome.runtime.onMessage.addListener(function (msg, _, sendResponse) {
  ...
  if (msg.type === MessageType.queryChat) {
    try {
      const msgEleList = document
        .querySelector('#chat #chatframe')
        .contentWindow.document.querySelectorAll('span#message');
      // chrome.runtime.sendMessage({
...
function filterChat(filterRegex = '[a-zA-Z]') {
  const chatIframeDocument =
    document.querySelector('#chat #chatframe').contentWindow.document;
  const chatRendererNodes = chatIframeDocument.querySelectorAll(
  ...
  chatRendererNodes.forEach((node, idx) => {
    ...
    if (msgNode.textContent.match(new RegExp(filterRegex))) {
      return;
    }
    node.remove();
  });
